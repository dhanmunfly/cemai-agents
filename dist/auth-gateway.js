...
app.post('/api/v1/auth/login', (req, res) => {
    ...
    logger_1.logger.info('Login attempt', { username, hasPassword: !!password });
    // Demo credentials for development
    if (username === 'operator@cemai.com' && password === 'password123') {
        const token = 'mock-jwt-token-' + Date.now();
        const refreshToken = 'mock-refresh-token-' + Date.now();
        logger_1.logger.info('Login successful', { username, token: token.substring(0, 20) + '...' });
        res.status(200).json({
            token,
            refreshToken,
            user: {
        ...
        logger_1.logger.warn('Login failed', { username, reason: 'Invalid credentials' });
        res.status(401).json({
...
app.post('/api/v1/auth/refresh', (req, res) => {
    logger_1.logger.info('Token refresh request');
    res.status(200).json({
...
app.use((err, req, res, next) => {
    logger_1.logger.error(`Auth Gateway Error: ${err.message}`, {
        method: req.method,
        path: req.path,
        error: err.message,
        stack: err.stack
...
// Start the server
app.listen(port, () => {
    logger_1.logger.info(`CemAI Auth Gateway listening on port ${port}`);
